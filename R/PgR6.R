...
PgR6 <- R6Class('PgR6',
                ...
                public = list(
                  ...
                  write_pangenome = function(dir = "pangenome", force = FALSE){
                    ...
                    data <- as.data.frame(data[act, -4L]) #Don't include "gid" column (4) as it's generated by pagoo
                    write_tsv(x = data,
                              path = paste0(dir, "data.tsv"),
                              col_names = TRUE)
                    rm(data)
                    # Clusters
                    write_tsv(as.data.frame(self$clusters),
                              path = paste0(dir, "clusters.tsv"),
                              col_names = TRUE)
                    # Organisms
                    write_tsv(as.data.frame(self$organisms),
                              path = paste0(dir, "organisms.tsv"),
                              col_names = TRUE)
