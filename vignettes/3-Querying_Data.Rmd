...
```
As you can see, changing the core level from 100% to a less stringent 99% cause in increase in the number of core genes from 1553 to 1623, and a concomitant decrease in shell genes from 511 to 441. This means that 70 genes migrated from shell to core when relaxing the core level to 99%. Now this changes remain in the object for subsequent analysis, or can be reverted by setting the core level again at the original value.
