...
In order to generate a `pagoo` object, the only mandatory data structure is a `data.frame` which has to contain 3 basic columns. Let's ilustrate it with an toy dataset.
...
All this stuff with preparing data and loading classes seems difficult and time-consuming, but in real life working datasets this will be rarely needed. We are explaning it here to provide full details about how the software works, but this package also provides functions to automatically read-in output files from pangenome reconstruction softwares into `pagoo`, avoiding any formating or manipulation of data.

For pangenome reconstruction you can use the software of your choice. For example, since last years <a [roary](https://sanger-pathogens.github.io/Roary/) (Page et al., 2015) has been the standard and most cited software for pangenome reconstruction. More recently, other good software has emerged like [PanX](http://pangenome.tuebingen.mpg.de) (Ding et al., 2018), [PRIATE](https://github.com/SionBayliss/PIRATE) (Bayliss et al., 2019), [micropan](https://github.com/cran/micropan) (Snipen & Liland, 2015), [PEPPA](https://github.com/zheminzhou/PEPPA) (Zhou & Achtman, 2020), [panaroo](https://github.com/gtonkinhill/panaroo) (Tonkin-Hill et al., 2020), among others. Also, we have created our own pangenome reconstruction software called [pewit](https://github.com/iferres/pewit) (Ferrés et al., still unpublished), which automatically generates a `pagoo` object to perform downstream analyses. To use `pagoo` from the outputs produced by most of the other software tools mentioned above, we provide specific functions to read-in pangenome data.
...
```{r}
p$write_pangenome(dir = "pangenome")
list.files("pangenome")
```
...
```{r, eval=FALSE}
p$save_pangenomeRDS(file = "pangenome.RDS")
p2 <- load_pangenomeRDS("pangenome.RDS")
```
This method is more stable (compatible between `pagoo` versions), secure (uses the same metadata classes, and precision isn't lost), and convinient (the exact state of the object saved is restored, keeping dropped organisms/genes/clusters hidden, available to be recovered, and other object state configuration, e.g. `core_level`, is also saved).
