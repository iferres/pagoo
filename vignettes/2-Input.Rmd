...
All the above stuff with preparing data and loading classes seems difficult and time-consuming, but in real life working datasets this will be rarely needed. We are explaning it here to provide full details about how the software works, but this package also provides functions to automatically read-in output files from pangenome reconstruction softwares into `pagoo`, avoiding any formating or manipulation of data.

Currently `pagoo` supports input from [roary](https://sanger-pathogens.github.io/Roary/) (Page et al., 2015), which has been the standard and most cited software for pangenome reconstruction. To work with `roary`'s ouput, please refer to `?roary_2_pagoo` documentation. You will only need the `.gff` files, and the `gene_presence_absence.csv` file.
...
More recently, other good software has emerged like [PanX](http://pangenome.tuebingen.mpg.de) (Ding et al., 2018), [PRIATE](https://github.com/SionBayliss/PIRATE) (Bayliss et al., 2019), [micropan](https://github.com/cran/micropan) (Snipen & Liland, 2015), [PEPPA](https://github.com/zheminzhou/PEPPA) (Zhou & Achtman, 2020), [panaroo](https://github.com/gtonkinhill/panaroo) (Tonkin-Hill et al., 2020), among others. We plan to provide support to most of them in the future, although some of them already provide options to output its results as roary's, so in theory you could use them as `pagoo`'s input.
